...
//! In integration mode (`OPSENSE_INTEGRATION=1`, chỉ export trong
//! `integration.yml`), failure panics so the workflow cannot silently go
//! green. On local dev / `ci.yml` without compose, skip gracefully.
